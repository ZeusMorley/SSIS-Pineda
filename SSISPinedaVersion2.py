...
def edit_course():
	...
	selected = select_course.get()
	query = "SELECT * FROM courses WHERE course_code= ?"
	c.execute(query, (selected,))
	...
	query2 = "DELETE from courses WHERE course_code= ?"
	c.execute(query2, (selected,))
	select_course.delete(0,END)
...
def student_keys():
	...
	students_key_list.geometry("300x300")
...
def edit_student():
	...
	submit_btn.grid(row=8, column=1, columnspan=2, pady=10, padx=(0,40), ipadx=40)
...
def list_down_students():
	...
	students_list.geometry("600x300")
...
def submit():
	...
	course.set(value="Select a Course")
...
select_course_label = Label(root, text="INPUT COURSE CODE", 
	font=('Arial', 10, 'bold'), fg='#35d17c', bg='#252525')
select_course_label.grid(row=19,column=0, pady=(2,0))
