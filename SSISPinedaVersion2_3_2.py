...
def student_keys():
	...
	students_key_list.geometry("300x400")
	students_key_list.resizable(width=False, height=False)
	students_key_list.configure(bg='#252525')

	listbox = tk.Listbox(students_key_list, width=50)
	listbox.config(font=('Arial', 12, 'bold'))
	listbox.config(bg='#252525', fg='#35d17c')
	listbox.pack(side=tk.LEFT, fill=tk.BOTH)

	scrollbar = tk.Scrollbar(students_key_list)
	scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
	...
	for record in records:
		listbox.insert(tk.END, str(record[7]) + " - " + str(record[2]) + ", " + str(record[0]) + " " + str(record[1]) + "\n")
...
def delete_student():
	...
	if records:
		...
		messagebox.showerror("Student Key Error", "The Student Key does not exist. \n Please refer to the Student Keys list to choose a student to delete.")
		return
	conn.commit()
	conn.close()
...
def edit_student():
	...
	if records:
		...
		messagebox.showerror("Student Key Error", "The Student Key does not exist. \n Please refer to the Student Keys list to choose a student to edit.")
		return	
